...
1. **엔진 독립성**: Apache POI든 FastExcel이든, 비즈니스 코드는 바뀌지 않습니다. Classpath에 어떤 엔진이 있느냐에 따라 자동으로 감지하고 위임합니다. 엔진 교체는 `build.gradle.kts`의 한 줄을 바꾸는 것으로 끝납니다.

2. **스트리밍 우선**: 10만 행이든 100만 행이든, 메모리 사용량은 일정합니다. `Sequence` 기반의 스트리밍 처리를 기본으로 설계했기 때문에, 대용량 데이터를 다루더라도 OOM 걱정 없이 안정적으로 동작합니다.

3. **확장 가능한 추상화**: DSL이 제공하지 않는 엔진 고유 기능(Freeze Pane, Auto Filter, Sheet Protection 등)은 `nativeSheet<T>` / `nativeWorkbook<T>` 확장 포인트를 통해 언제든 접근할 수 있습니다. 추상화의 편의성과 네이티브의 강력함, 둘 다 포기하지 않습니다.
...
| 피벗 테이블 | ✅ | ❌ |
| 기존 파일 읽기 / 수정 | ✅ | ❌ |
| 서버리스 / 컨테이너 적합성 | ⚠️ 무거움 | ✅ 경량 |

### 🐘 Apache POI (SXSSF)
*업계 표준 & 풍부한 기능*
- **추천 대상**: 복잡한 스타일링, 수식 평가, 차트/그래프, 피벗 테이블 등 엑셀의 고급 기능이 필요한 보고서.
- **강점**: 20년 이상 축적된 압도적인 호환성과 방대한 기능셋. 기존 Excel 파일을 읽거나 수정하는 작업도 지원합니다.
- **고려 사항**: 전이 의존성이 많아 전체 JAR 크기가 ~30MB에 달하며, 서버리스/컨테이너 환경에서는 콜드 스타트에 영향을 줄 수 있습니다.

### 🏎️ FastExcel
*고성능 & 경량*
- **추천 대상**: 수백만 행의 대용량 데이터 처리, 밀리초 단위의 성능과 메모리 효율이 중요한 고동시성 환경.
- **강점**: ~150KB의 극도로 가벼운 의존성. POI 대비 월등히 빠른 처리 속도와 최소한의 메모리 점유율을 가집니다. Lambda, Cloud Run 등 서버리스 환경에 최적입니다.
- **고려 사항**: 수식 평가·차트·피벗 테이블·기존 파일 수정 같은 POI의 고급 기능은 지원하지 않습니다.

**왜 KExcel인가?** 처음에는 풍부한 기능을 위해 POI로 시작했다가, 데이터가 늘어나 성능 병목이 발생하면 **빌드 설정의 의존성 한 줄만 바꿔서 FastExcel로 즉시 전환**할 수 있습니다.
